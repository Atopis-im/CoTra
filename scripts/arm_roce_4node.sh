...
# 4-node variant of arm_roce_8node.sh.
# Generated by adapting scripts/arm_roce_8node.sh: NODE0..NODE3 IPs,
# write_config emits 4 lines, detect_local_node scans 4 IPs,
# CMake is configured with -DCOTRA_MACHINE_NUM=4.
...
#   WARMUP_RUNS  — untimed warmup passes before timed runs (0=skip, default 1)
#   WARMUP_EF    — ef for warmup (0 = use max ef from SEARCH_EF_LIST)
#   NUM_RUNS     — timed repetitions per ef; QPS reported as median (default 10, 1=old single-shot)
...
usage() {
  printf '%s\n' \
    "Usage:" \
    "  NODE1_RDMA_IP=<ip1> ... NODE3_RDMA_IP=<ip3> bash scripts/arm_roce_4node.sh <mode> [options]" \
    "" \
    "Modes:" \
    "  check    Check toolchain, headers, RDMA, routes, dataset, and shared path." \
    "  build    Run check, configure CMake for 4 nodes, and compile." \
    "  index    Run the distributed GIST index build across 4 nodes." \
    "  search   Search the completed index across 4 nodes." \
    "" \
    "Important options:" \
    "  --node0-rdma-ip IP .. --node3-rdma-ip IP   RoCE IPs for the 4 nodes." \
    ...
    "  --max-degree R       Graph max out-degree (M/R). 4node default: 48" \
    "  --build-l L          Build queue size / efC. 4node default: 500" \
    ...
    "  --search-ef-list L   Comma-separated ef values to sweep (e.g. 50,100,200)." \
    "                       Default: empty (use built-in list 3..700)" \
    ...
    "" \
    "Start node 0 first on every mode, then nodes 1..3."
}
...
if [[ -z $BUILD_DIR ]]; then
  BUILD_DIR="${REPO_ROOT}/build-arm-4node-${HOST_SHORT}"
fi
if [[ -z $OUTPUT_DIR ]]; then
  OUTPUT_DIR="${DATASET_DIR}/cotra_4node_index"
fi
CONFIG_FILE="${BUILD_DIR}/cotra_4node.conf"
...
runtime_preflight() {
  ...
  # only once the RDMA barrier starts is very costly for multi-node runs).
...
run_search() {
  ...
  # 改为只 warn 不 die
  local required_index="${OUTPUT_DIR}/merged_index${LOCAL_NODE_ID}_final_scala.index"
  if [[ ! -f $required_index ]]; then
      warn "Index file not found: $required_index (this may be expected for non-scala graph types)"
  fi  
  reset_metadata
  rdma_arguments
