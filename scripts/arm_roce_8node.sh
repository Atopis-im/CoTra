...
# 8-node variant of arm_roce_2node.sh.
# Generated by adapting scripts/arm_roce_2node.sh: NODE0..NODE7 IPs,
# write_config emits 10 lines, detect_local_node scans 8 IPs,
# CMake is configured with -DCOTRA_MACHINE_NUM=8.
...
usage() {
  printf '%s\n' \
    "Usage:" \
    "  NODE1_RDMA_IP=<ip1> ... NODE7_RDMA_IP=<ip7> bash scripts/arm_roce_8node.sh <mode> [options]" \
    "" \
    "Modes:" \
    "  check    Check toolchain, headers, RDMA, routes, dataset, and shared path." \
    "  build    Run check, configure CMake for 8 nodes, and compile." \
    "  index    Run the distributed GIST index build across 8 nodes." \
    "  search   Search the completed index across 8 nodes." \
    "" \
    "Important options:" \
    "  --node0-rdma-ip IP .. --node7-rdma-ip IP   RoCE IPs for the 8 nodes." \
...
if [[ -z $BUILD_DIR ]]; then
  BUILD_DIR="${REPO_ROOT}/build-arm-8node-${HOST_SHORT}"
fi
if [[ -z $OUTPUT_DIR ]]; then
  OUTPUT_DIR="${DATASET_DIR}/cotra_8node_index"
fi
CONFIG_FILE="${BUILD_DIR}/cotra_8node.conf"
