...
# Comma-separated search ef list override (e.g. "50,100,200").
# Empty → use the C++ built-in default sweep list.
SEARCH_EF_LIST="${SEARCH_EF_LIST:-}"
# Measurement control: warmup + repeated runs (median QPS).
...
while (($# > 0)); do
  case "$1" in
    ...
    --res-knn)
      ...
      ;;
    --search-ef-list)
      SEARCH_EF_LIST=${2:?missing value for --search-ef-list}
      shift 2
      ;;
    --num-runs|--num_runs)
      NUM_RUNS=${2:?missing value for --num-runs}
      shift 2
      ;;
    --warmup-runs|--warmup_runs)
      ...
      ;;
    --warmup-ef|--warmup_ef)
      WARMUP_EF=${2:?missing value for --warmup-ef}
      shift 2
      ;;
    --app-mode)
...
run_search() {
  ...
  local command=(
    ...
    --warmup_ef "${WARMUP_EF:-0}"
    --num_runs "${NUM_RUNS:-10}"
    ${SEARCH_EF_LIST:+--search_ef_list "$SEARCH_EF_LIST"}
    "${RDMA_ARGS[@]}"
  )
