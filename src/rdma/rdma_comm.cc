...
void RdmaCommunication::send_term(
  ...
  for (int m = 0; m < MACHINE_NUM; m++) {
    if (m == my_machine_id) {
      ...
      query_load += q_query_load;
      query_lat_sum += cumu_lat_us;
      // Include self dist_cnt so all_computation_cnt is a TRUE all-reduce
      // (Σ over all MACHINE_NUM ranks == global G on every rank).
      // Without this line the self-branch adds correct/total/query_load/
      // query_lat_sum but NOT all_computation_cnt, so each rank ends up
      // with all_computation_cnt = Σ(7 peers) = G - own_dist_cnt, which
      // only coincides across ranks when every rank's dist_cnt is equal.
      // That holds in config A (1 proc/node, dedicated NIC) but NOT in
      // config B (2 procs/node sharing NIC/CPU, larger per-rank dist_cnt
      // spread), which is exactly the observed second-exchange divergence.
