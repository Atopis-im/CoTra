...
class RdmaCommunication {
 public:
  ...
  // �── Term epoch (round id) ────────────────────────────────────────
  // TERMINATION 消息的跨轮竞态修复: 快节点发出的下一轮 term 可能在慢节点
  // 仍在当前轮 standby 时到达, 被误当作当前轮 term 消费掉, 导致:
  //   (a) 当前轮聚合值 (correct/total/query_load/all_computation_cnt)
  //       被下一轮 payload 污染;
  //   (b) 下一轮 term 交换永远等不到该消息 -> 挂死.
  // 修复: 每条 TERMINATION 携带 round id (term_round).
  //   round == term_round  -> 正常应用 (与修复前行为一致);
  //   round >  term_round  -> 暂存 future_terms, 等本节点进入该轮后应用;
  //   round <  term_round  -> 丢弃 (陈旧消息).
  // 所有节点必须以相同的次数/顺序调用 reset_term() (即 initTerm), 才能保证
  // round 编号一致 — 当前代码里 warmup/正式轮/第二次 term 交换在所有节点
  // 上完全对称, 满足此前提.
  std::atomic<uint64_t> term_round{0};
  ...
  struct FutureTerm {
    ...
    size_t query_load;
    double cumu_lat_us;
    size_t computation_cnt;
  };
  std::deque<FutureTerm> future_terms;
  std::mutex future_term_mtx;  // poll_recv 多线程并发, 仅 future 路径加锁
...
// 1. Send NodeResult to original machine (NodeResult queue).
  ...
  // 返回 true=正常收到query, false=收到leader的TERMINATION(member应退出)
  bool poll_b2_start(char *query_data, size_t &ef, uint32_t &qid);
