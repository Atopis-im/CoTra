...
class ScalaSearch : public AlgorithmInterface<dist_t> {
 public:
  ...
  // 距离计算次数 (在 space_l2.h 的 L2CountingWrapper 里累加, 线程安全).
  // 覆盖 cotra(ScalaANN_v3) + shard(B2) 两种模式: 二者都经 L2Space 的
  // fstdistfunc_, 而该函数已被计数包装器替换.
  //   resetDistComputations() — 搜索前清零
  //   getDistComputations()   — 搜索后读取本进程累计值
  // 注: 这是"本机本地分片"的距离计算次数; 全集群总数 = 各节点之和.
  void resetDistComputations() {
    hnswlib::g_l2_dist_computations.store(0, std::memory_order_relaxed);
  }
  uint64_t getDistComputations() const {
    return hnswlib::g_l2_dist_computations.load(std::memory_order_relaxed);
  }

  // 全局距离计算次数 (term 协议聚合后).
  // sendTerm() 将本机 dist_cnt 发给所有节点, 各节点累加到
  // rdma_comm.all_computation_cnt. check_term() 返回 true 后,
  // all_computation_cnt = Σ 全部节点 dist_cnt.
  // 注意: 对 cotra, 第一次 sendTerm 在 standby 之前调用, 故首次聚合值
  //       不含 standby 阶段距离; 如需精确值须做第二次 term 交换
  //       (见 exec_query.h 末尾的 [dist_cnt_global] 输出).
  //       对 shard(B2), termStandBy 不算距离, 单次即精确.
  uint64_t getGlobalDistComputations() const {
    return (uint64_t)rdma_comm.all_computation_cnt;
  }
   ...
   */
  // 返回 true=正常分发/接收了一个query, false=member收到leader的term(应退出循环)
  bool dispatch_query(void *query_data, uint32_t qid, size_t k) {
    if (rdma_param.machine == LEADER) {
      ...
      uint32_t qid;
      return rdma_comm.poll_b2_start((char *)query_data, ef_, qid);
    }
  }
...
// remote memory
  ...
  void printQueryBatchInfo(float time) {
    ...
    if (!header_printed) {
#ifdef PROF_COMPUTATION
      printf("[node=%u] ef \t recall \t avg_lat(ms) \t qps \t comp_cnt\n",
             (unsigned)rdma_param.machine_id);
#else
      printf("[node=%u] ef \t recall \t avg_lat(ms) \t qps\n",
             (unsigned)rdma_param.machine_id);
#endif
      header_printed = true;
    }
#ifdef PROF_COMPUTATION
    printf(
        "[node=%u] %d \t %.5f \t %.4f \t %.3f \t %llu \n",
        (unsigned)rdma_param.machine_id, ef_, recall, avg_lat,
        qps, all_comp_cnt);
#else
    printf(
        "[node=%u] %d \t %.5f \t %.4f \t %.3f \t \n",
  ...
  void printQueryBatchInfoSave(float time, std::ofstream &out) {
    ...
    float avg_lat = rdma_comm.query_lat_sum / rdma_comm.query_load / 1000;
    float qps = 1000000.0 * rdma_comm.query_load / time;
    printf("[node=%u] -------------- Overall info --------------\n",
           (unsigned)rdma_param.machine_id);
    printf(
        "[node=%u] correct: %llu total: %llu query load: %llu\n",
        (unsigned)rdma_param.machine_id, rdma_comm.correct,
  ...
  void sendTerm(size_t correct, size_t total, size_t query_load, double cumu_lat_us = 0.0) {
    // 始终通过 term 协议发送 dist_cnt (g_l2_dist_computations),
    // 让 check_term() 后 rdma_comm.all_computation_cnt = 全局 Σ dist_cnt.
    // (PROF_COMPUTATION 的 computation_cnt 统计的是同一件事, 但仅在
    // ifdef 下生效; dist_cnt 始终生效, 故统一用 dist_cnt.)
    size_t dist_cnt = (size_t)getDistComputations();
    rdma_comm.send_term(rdma_param.machine_id, correct, total,
      query_load, cumu_lat_us, dist_cnt);
#ifdef PROF_COMPUTATION
    computation_cnt = 0;
#endif
    return;
