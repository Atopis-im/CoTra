...
struct VectorBuffer {
  ...
  // Free-list of available read buffer ids for this thread's pool.  This is
  // mutated by two kinds of threads:
  //   * the owning thread, which pops a buffer id in post_read() before posting
  //     an RDMA read, and
  //   * any thread that finishes consuming a remote vector, which calls
  //     release_cache(buffer_id, owner_thread) and pushes the id back into the
  //     *owner's* list (see rdma_comm.cc release_cache / post_read).
  // A std::deque has no internal synchronisation, so concurrent push_back +
  // pop_front is a data race that corrupts the free list, leaking/duplicating
  // ids until post_read() pops from an empty deque (segfault, since the assert
  // below is compiled out under -DNDEBUG/Release).  The race rate grows with
  // the number of threads that return buffers to a given owner, which is why
  // 8/24 threads (single NUMA, little cross-thread return) were stable but
  // 48/72 threads (multi-NUMA, stealOutsideSocket) segfaulted.  Guard every
  // push/pop with buff_id_mtx.
  std::deque<int> buff_id_list;
  mutable std::mutex buff_id_mtx;
