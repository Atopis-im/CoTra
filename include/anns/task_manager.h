 ...
 public:
  ...
  void clear() {
    for (uint32_t m = 0; m < MACHINE_NUM; m++) {
      ...
      // Previously these were NOT cleared here, so tasks/results from a
      // previous ef iteration (or a previous run within the same ef) leaked
      // into the next one.  Across num_runs iterations this accumulation
      // made later runs progressively slower (the "runs 6-10 slow down"
      // pattern, CV ~38% at 72 threads).  Clearing all per-run queues at
      // the start of each iteration removes the accumulation.
      node_task_queue[m].clear();
      node_result_queue[m].clear();
      noncore_sync_msg[m].clear();
      compute_queue[m].clear();
    }
    cur_task_queue.clear();
    next_task_queue.clear();
    cur_node_queue.clear();
